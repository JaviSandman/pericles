"""
Senado — Resultados por nivel geográfico y evolución temporal
Fuentes: tipo_06.parquet, tipo_03.parquet

Niveles: Nacional · CCAA · Provincia · Municipio
Nota: en el Senado los senadores se asignan a nivel provincial (circunscripción).
"""
import sys
...
st.set_page_config(page_title="Senado", page_icon="🏛️", layout="wide")
st.title("🏛️ Senado de España")
...
with tab_evo:
    ...
    candidates = ["PP", "PSOE", "Cs", "VOX", "IU", "UPYD", "UCD", "AP", "CDS"]
...
with tab_mapa:
    ...
    if _map_nivel == "CCAA":
        ...
        render_election_map(
            ...
            nacional_seat_df=(
                df_all[
                    (df_all["anio"] == int(_map_conv[:4])) &
                    (df_all["mes"]  == int(_map_conv[5:]))
                ]
                .groupby(["provincia_cod", "partido"], as_index=False)
                .agg(senadores=("candidatos_obtenidos", "sum"))
                .pipe(lambda d: d[d["senadores"] > 0])
                if _map_nivel == "Nacional" else None
            ),
            nacional_seat_col="senadores",
            nacional_seat_label="senadores",
            key="senado_mapa_tab",
            height=560,
        )